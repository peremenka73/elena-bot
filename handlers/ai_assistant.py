...
async def process_ai_question(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    # Индикатор «печатает» в Telegram сам гаснет через ~5 секунд, а DeepSeek может
    # отвечать дольше — поэтому обновляем его в фоне, пока не придёт ответ,
    # чтобы пользователь не подумал, что бот завис.
